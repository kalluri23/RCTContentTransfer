...
//    [self.activityIndicator showAnimated:YES];
    ...
    self.waitTillPhotoCompleteDataIsCollected = YES;
    self.waitTillVideoCompleteDataIsCollected = YES;
...
- (void)cancelInProcessDataCollectionIfUserNotSelected {
    ...
    for (NSIndexPath *indexpath in selectedRows) {
        ...
        if (indexpath.row == CTTransferItemsTableBreakDown_RemindersOrAudios && [[CTUserDevice userDevice].phoneCombination isEqualToString:IOS_Andriod]) {
            audioNotSelected = NO;
        }
    }
...
- (IBAction)handleSelectAllButtonTapped:(id)sender {
    ...
    if ([CTContactsManager contactsAuthorizationStatus] == CTAuthorizationStatusAuthorized && [CTDataCollectionManager sharedManager].getNumberOfContacts > 0) {
    ...
    if ([[CTUserDevice userDevice].phoneCombination isEqualToString:IOS_IOS]) { // reminder
        if ([CTEventStoreManager reminderAuthorizationStatus] == CTAuthorizationStatusAuthorized && [CTDataCollectionManager sharedManager].getNumberOfReminders > 0){
    ...
    if ([CTEventStoreManager calendarAuthorizationStatus] == CTAuthorizationStatusAuthorized && [CTDataCollectionManager sharedManager].getNumberOfCalendars > 0) {
...
- (void)startDisPlayCollectingDataDialog {
    ...
    if (self.photoSelected && self.waitTillPhotoCompleteDataIsCollected) {
        [items addObject:CTLocalizedString(CT_PHOTO_STRING, nil)];
    }
    if (self.videoSelected && self.waitTillVideoCompleteDataIsCollected) {
        [items addObject:CTLocalizedString(CT_VIDEO_STRING, nil)];
    }
...
- (void)stopDisplayPhotoAndVideoDataConnectingDialog {
...
//        [weakSelf setItem:METADATA_ITEMLIST_KEY_PHOTOS withCount:[CTDataCollectionManager sharedManager].getNumbersOfPhotos withSize:[CTDataCollectionManager sharedManager].getSizeOfPhotos];
    ...
    if (self.photoSelected && self.userClickedTransferBtn && tempPhotoCompeletionStatus) {
        [self stopDisplayPhotoAndVideoDataConnectingDialog];
    }
}
...
- (void)audioFetchingIsCompleted {
    dispatch_async(dispatch_get_main_queue(), ^{
        self.audioNumberLbl.text = [self formattedCountText:CTLocalizedString(CT_AUDIO, nil) WithCount:[[CTDataCollectionManager sharedManager] getNumberOfAudios]];
        self.audioDataSizeLbl.text = [NSString formattedDataSizeTextInTransferWhatScreen:[[CTDataCollectionManager sharedManager] getSizeOfAudio]];
//        [self setItem:METADATA_ITEMLIST_KEY_AUDIOS withCount:[[CTDataCollectionManager sharedManager] getNumberOfAudios] withSize:[[CTDataCollectionManager sharedManager] getSizeOfAudio]];
    ...
    if (self.audioSelected && self.userClickedTransferBtn) { // fetch complete callback called after user select transfer with current type
        [self stopDisplayPhotoAndVideoDataConnectingDialog];
    }
}
