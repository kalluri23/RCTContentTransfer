...
@interface CTStartedViewController ()<UITextFieldDelegate, CalendarImportDelegate, PhotoStoreDelegate, CTAppListDelegate> {
...
@property (weak, nonatomic) IBOutlet CTCommonBlackButton *startButton;
...
@implementation CTStartedViewController
...
- (void)viewDidLoad {
    ...
    self.title = CTLocalizedString(CT_GET_STARTED_NAV_TITLE, nil);
...
//    self.progressView.hidden = YES;
    ...
    if (STANDALONE == 1) { // Setup for standalone
        ...
        self.secondaryTextLabel.text = CTLocalizedString(CT_GET_STRATED_PRIMARY_MESSGE, nil);
        // Attributed link
        NSMutableAttributedString *attiStr = [[NSMutableAttributedString alloc] initWithString:self.clickableLabel.text];
        NSString *tncString = CTLocalizedString(CT_TNC_LINK, nil);
        ...
        // Privacy Policy Link
        NSString *privacyPolicyText = CTLocalizedString(CT_PRIVACY_POLICY_HYPERLINK, nil);
        //        NSString *seperatorText = @"   |   ";
        NSString *aboutText = CTLocalizedString(CT_ABOUT_HPERLINK, nil);
        ...
        // PP & About attributed link
        NSMutableAttributedString *attiStr = [[NSMutableAttributedString alloc] initWithString:NSLocalizedStringFromTableInBundle(CT_ABOUT_HPERLINK, @"Localizable",   [NSBundle bundleWithIdentifier: @"com.vzw.contentTransfer.framework.bundle"], nil)];
        // About Link
        self.aboutLinkRange = NSMakeRange(0, attiStr.length);
        [attiStr addAttribute:NSLinkAttributeName value:kAboutAppUrl range:self.aboutLinkRange];
        [attiStr addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStyleNone] range:self.aboutLinkRange];
        [attiStr addAttribute:NSUnderlineColorAttributeName value:[UIColor clearColor] range:self.aboutLinkRange];
        [attiStr addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:75/255.0 green:145/255.0 blue:222/255.0 alpha:1.0] range:self.aboutLinkRange];
...
- (void)viewDidLayoutSubviews {
    ...
    if (STANDALONE && IS_STANDARD_IPHONE_4_OR_LESS) {
        ...
        self.primaryMessageLabel.font = [self.primaryMessageLabel.font fontWithSize:35.f];
    }
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [CTUserDefaults sharedInstance].errorLivePhotoList = nil; // Clear the error live photo list.
...
- (void)showLowBatteryScreen {
    ...
    batteryErrorScreen.primaryErrorText = CTLocalizedString(ALERT_TITLE_PLUG_IN_AND_CHARGE_UP, nil);
    batteryErrorScreen.secondaryErrorText = CTLocalizedString(ALERT_MESSAGE_BATTERY_WARNING_MESSAGE, nil);
    batteryErrorScreen.rightButtonTitle = CTLocalizedString(BUTTON_TITLE_GOT_IT, nil);
...
- (IBAction)handleInfoButtonTapped:(id)sender {
    // Fetch build date
    NSString *stringDate  = BUILD_DATE;

    // Fetch build version
    NSString *stringBuild = @"";
#if FULL_VERSION == 1
    // For QA-Release and Debug build, for full build number.
    stringBuild = BUILD_VERSION_FULL;
#else
    stringBuild = BUILD_VERSION;
#endif
    
    if (USES_CUSTOM_VERIZON_ALERTS) {
        [CTVerizonAlertCreateFactory showSingleButtonsAlertWithTitle:kDefaultAppTitle
                                                             context:[NSString stringWithFormat:CTLocalizedString(CT_GET_STARTED_INFO_ALERT_CONTEXT, nil), stringBuild, stringDate]
                                                             btnText:CTLocalizedString(CT_OK_ALERT_BUTTON_TITLE, nil)
                                                             handler:nil
...
- (void)checkUnsavedDataForDevice {
    ...
    [self displayUnsavedDataRequestDialogSaving:^{
        ...
        _alertView = [[CTCustomAlertView alloc] initCustomAlertViewWithText:CTLocalizedString(CT_SAVING_DATA, nil) withOritation:CTAlertViewOritation_VERTICAL];
        [_alertView show:^{
            [weakSelf saveFiles];
        }];
    } delete:^{
        // should delete unsaved data
        _alertView = [[CTCustomAlertView alloc] initCustomAlertViewWithText:CTLocalizedString(CT_DELETE_DATA, nil) withOritation:CTAlertViewOritation_VERTICAL];
        [_alertView show:^{
...
- (void)displayUnsavedDataRequestDialogSaving:(void(^)(void))save
    ...
    if (hasUnsavedVcard || hasUnsavedReminder || hasUnsavedPhoto || hasUnsavedVideo || hasUnsavedApps) { // if has any unsaved data
        ...
        NSString *dialogTitle = CTLocalizedString(kDefaultAppTitle, nil);
        
        NSString *title = CTLocalizedString(CT_DETECTED_UNSAVED_DATA_ALERT_CONTEXT, nil);
        ...
        if (hasUnsavedVcard) {
            [attributedStr appendAttributedString:[self makeAttributed:[NSString stringWithFormat:@"\n•  %@: %ld", CTLocalizedString(CT_CONTACTS, nil), (long)unsavedVcardNum] withFont:[CTMVMFonts mvmBookFontOfSize:14.f] style:leftParagraphStyle]];
        }
        
        if (hasUnsavedPhoto) {
            [attributedStr appendAttributedString:[self makeAttributed:[NSString stringWithFormat:@"\n•  %@: %ld",CTLocalizedString(CT_PHOTOS, nil), (long)unsavedPhotoNum] withFont:[CTMVMFonts mvmBookFontOfSize:14.f] style:leftParagraphStyle]];
        }
        
        if (hasUnsavedVideo) {
            [attributedStr appendAttributedString:[self makeAttributed:[NSString stringWithFormat:@"\n•  %@: %ld", CTLocalizedString(CT_VIDEOS, nil), (long)unsavedVideoNum] withFont:[CTMVMFonts mvmBookFontOfSize:14.f] style:leftParagraphStyle]];
        }
        
        if (hasUnsavedReminder) {
            [attributedStr appendAttributedString:[self makeAttributed:[NSString stringWithFormat:@"\n•  %@: %ld", CTLocalizedString(CT_REMINDERS, nil), (long)unsavedReminderListNum] withFont:[CTMVMFonts mvmBookFontOfSize:14.f] style:leftParagraphStyle]];
        }
        ...
        if (hasUnsavedCalendar) {
            [[[VZCalenderEventsImport alloc] init] getTotalCalendarEventCount:^(NSInteger eventCount) { // get the event count, not the file number
                ...
                [attributedStr appendAttributedString:[self makeAttributed:[NSString stringWithFormat:@"\n•  %@: %ld",CTLocalizedString(CT_CALENDERS, nil), (long)unsavedCalendarNum] withFont:[CTMVMFonts mvmBookFontOfSize:14.f] style:leftParagraphStyle]];
                
                if (hasUnsavedApps) {
                    [attributedStr appendAttributedString:[self makeAttributed:[NSString stringWithFormat:@"\n•  %@",CTLocalizedString(CT_APPS_LIST, nil)] withFont:[CTMVMFonts mvmBookFontOfSize:14.f] style:leftParagraphStyle]];
                }
...
- (void)buildAlertWithAttributedTitle:(NSAttributedString *)title andAttributedText:(NSAttributedString *)text saving:(void(^)(void))save delete:(void(^)(void))delete  {
    ...
    if (USES_CUSTOM_VERIZON_ALERTS)  {
        [CTVerizonAlertCreateFactory showTwoButtonsAlertWithAttributedTitle:title attributedContext:text cancelBtnText:CTLocalizedString(CT_DELETE_ALERT_BUTTON_TITLE, nil) confirmBtnText:CTLocalizedString(CT_SAVE_ALERT_BUTTON_TITLE, nil) confirmHandler:^(CTVerizonAlertViewController *alertVC){
            save();
        ...
        if ([UIAlertController class] != nil) { // only iOS 8 and above can be receiver side, so no UIAlertView, only UIAlertController
            ...
            UIAlertAction *saveAction = [UIAlertAction actionWithTitle:CTLocalizedString(CT_SAVE_ALERT_BUTTON_TITLE, nil)
                                                                 style:UIAlertActionStyleDestructive
                                                               handler:^(UIAlertAction * _Nonnull action) {
                                                                   save();
                                                               }];
            UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:CTLocalizedString(CT_DELETE_ALERT_BUTTON_TITLE, nil)
...
- (void)deleteFiles {
    ...
    if (hasUnsavedVcard) { // Contacts
        ...
        if (!success) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [_alertView updateLbelText:[NSString stringWithFormat:@"%@ : %ld / %ld", CTLocalizedString(CT_DELETED_CONTACTS_MESSAGE, nil), (long)unsavedVcardNum, (long)unsavedVcardNum] oritation:CTAlertViewOritation_VERTICAL];
            });
        }
    ...
    if (hasUnsavedReminder) { // Reminders
        ...
        if (!success) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [_alertView updateLbelText:[NSString stringWithFormat:@"%@ : %ld / %ld", CTLocalizedString(CT_DELETED_REMINDERS_MESSAGE, nil), (long)unsavedReminderNum, (long)unsavedReminderNum] oritation:CTAlertViewOritation_VERTICAL];
            });
        }
    ...
    if (hasUnsavedCalendar) {
        ...
        for (NSString *file in unsavedCalendars) {
            ...
            if (!success) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_alertView updateLbelText:[NSString stringWithFormat:@"%@ : %d / %lu", CTLocalizedString(CT_DELETED_CALANDERS_MESSAGE, nil), ++calendarDeleteCount, (unsigned long)unsavedCalendars.count] oritation:CTAlertViewOritation_VERTICAL];
                });
            }
        }
    ...
    if (hasUnsavedPhoto) {
        ...
        for (NSString *file in unsavedPhotos) {
            NSDictionary * photoMetaData = (NSDictionary *)[photoDic objectForKey:file];
            ...
            if ([CTLivePhoto isCurrentImageLivePhoto:photoMetaData]) { // If it's live photo, remove video components
                error = nil;
                success = [fm removeItemAtPath:[[[NSString appRootDocumentDirectory] stringByAppendingPathComponent:@"VZTransferLivePhoto"] stringByAppendingPathComponent:[photoMetaData valueForKey:@"Resource"]] error:&error];
                if (!success) {
                    // Don't need to handle the delete fail, because next time user transfer, folder will be clear
                    DebugLog(@"delete failed! Error:%@", error.localizedDescription);
                }
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [_alertView updateLbelText:[NSString stringWithFormat:@"%@ : %d / %lu", CTLocalizedString(CT_DELETED_PHOTOS_MESSAGE, nil), ++photoDeleteCount, (unsigned long)unsavedPhotos.count] oritation:CTAlertViewOritation_VERTICAL];
            });
        }
    ...
    if (hasUnsavedVideo) {
        ...
        for (NSString *file in unsavedVideos) {
            ...
            if (!success) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_alertView updateLbelText:[NSString stringWithFormat:@"%@ : %d / %lu", CTLocalizedString(CT_DELETE_VIDEOS, nil), ++videoDeleteCount, (unsigned long)unsavedVideos.count] oritation:CTAlertViewOritation_VERTICAL];
                });
            }
        }
    ...
    if (hasUnsavedApps) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [_alertView updateLbelText:CTLocalizedString(CT_DELETE_APPS_LIST, nil) oritation:CTAlertViewOritation_VERTICAL];
        });
...
- (void)importVcardData:(NSData *)vcardData {
    ...
    vCardImport.completionHandler = ^(NSInteger contactNumber) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_CONTACTS, nil), (long)contactNumber, (long)unsavedVcardNum] oritation:CTAlertViewOritation_VERTICAL];
        });
    ...
    vCardImport.updateHandler = ^(NSInteger updateCount) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %d / %ld", CTLocalizedString(CT_SAVING_CONTACTS, nil), updateNumber, (long)unsavedVcardNum] oritation:CTAlertViewOritation_VERTICAL];
        });
    };
...
- (void)importReminder {
    ...
    reminderImport.completionHandler = ^(NSInteger totalReminderEventSaved, NSInteger totalReminderEventCount, NSInteger actualSavedListCount) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_REMINDERS, nil), (long)totalReminderEventSaved, (long)unsavedReminderNum] oritation:CTAlertViewOritation_VERTICAL];
        });
    ...
    reminderImport.updateHandler = ^(NSInteger reminderUpdateCount, NSInteger totalReminderEventCount) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_REMINDERS, nil), (long)reminderUpdateCount, (long)unsavedReminderNum] oritation:CTAlertViewOritation_VERTICAL];
        });
    };
...
- (void)importCalendar {
    ...
    [calendarImport createCalendarsSuccess:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_CALENDERS, nil), (long)unsavedCalendarNum, (long)unsavedCalendarNum] oritation:CTAlertViewOritation_VERTICAL];
        });
...
- (void)shouldUpdateCalendarNumber:(NSInteger)eventCount {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_CALENDERS, nil), (long)_calendarSavedCount, (long)unsavedCalendarNum] oritation:CTAlertViewOritation_VERTICAL];
    });
}
...
- (void)updateDuplicatePhoto:(NSArray<NSString *> *)URLs withPhotoInfo:(NSDictionary *)photoInfo withLocalIdentifier:(NSString *)localIdentifier success:(BOOL)success orError:(NSError *)error {
    ...
    if (success && localIdentifier) {
        ...
        [self.localDuplicatePhotoList setObject:localIdentifier forKey:[[URLs objectAtIndex:0] lastPathComponent]]; // Success then URL array always exists
    }
    
    dispatch_async(dispatch_get_main_queue(), ^{
        [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_PHOTOS, nil), (long)_totalNumber-(long)unsavedPhotoNum, (long)_totalNumber] oritation:CTAlertViewOritation_VERTICAL];
    });
...
- (void)updateDuplicateVideo:(NSString *)URL withVideoInfo:(NSDictionary *)videoInfo withLocalIdentifier:(NSString *)localIdentifier success:(BOOL)success orError:(NSError *)error {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [_alertView updateLbelText:[NSString stringWithFormat:@"%@: %ld / %ld", CTLocalizedString(CT_SAVING_VIDEOS, nil), (long)_totalNumber-(long)unsavedVideoNum, (long)_totalNumber] oritation:CTAlertViewOritation_VERTICAL];
    });
...
- (void)showReactNativeScreens {
    ...
    RCTRootView *rootView = [[RCTRootView alloc] initWithBundleURL:jsCodeLocation
                                                        moduleName:@"RCTContentTransfer"
                                                 initialProperties:nil
                                                     launchOptions:nil];
    rootView.backgroundColor = [[UIColor alloc] initWithRed:1.0f green:1.0f blue:1.0f alpha:1];
    
    UIViewController *reactiveViewController = [UIViewController new];
    reactiveViewController.view = rootView;
    [self presentViewController:reactiveViewController animated:YES completion:nil];
}
